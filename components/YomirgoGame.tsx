...
const YomirgoGame: React.FC = () => {
    ...
    const update = () => {
        ...
        if (keys['ArrowLeft'] || keys['KeyA']) { player.vx -= 1; player.facingRight = false; }
        if (keys['ArrowRight'] || keys['KeyD']) { player.vx += 1; player.facingRight = true; }
        player.vx = Math.max(Math.min(player.vx, MOVE_SPEED), -MOVE_SPEED);
        if ((keys['Space'] || keys['ArrowUp'] || keys['KeyW']) && player.isGrounded) {
            player.vy = JUMP_FORCE * 1.02; 
            player.isGrounded = false;
            createParticles(player.x + player.w/2, player.y + player.h, COLOR_BRAND_ORANGE, 5, 'explosion');
        }
        player.vy += GRAVITY * 0.85; player.vx *= FRICTION; player.x += player.vx; player.y += player.vy;
    ...
    const KeyCap = ({ label, size = 'w-10', delay = '0s' }: any) => (
        <div style={{ animationDelay: delay }} className={`${size} h-10 border-2 border-[#E57D25]/30 rounded flex items-center justify-center text-[#E57D25]/40 font-bold font-mono transition-all animate-key-glow`}>{label}</div>
    );
        ...
        <div className="relative rounded-xl overflow-hidden shadow-[0_0_20px_rgba(229,125,37,0.3)] border-4 border-[#333] bg-black max-w-full">
            <style>
                ...
                @keyframes pumpFlash { 
                    ...
                    50% { opacity: 1; transform: scale(1.1); text-shadow: 0 0 25px rgba(229,125,37,0.9), 0 0 10px rgba(229,125,37,1); } 
                    100% { opacity: 0; transform: scale(1.3); } 
                }
                .pump-transition { animation: pumpFlash 0.8s ease-out forwards; }
                `}
            </style>
            <canvas ref={canvasRef} width={CANVAS_WIDTH} height={CANVAS_HEIGHT} className="block w-full h-auto max-h-[85vh] object-contain" style={{ imageRendering: 'pixelated', maxWidth: '600px' }} />
            
            {/* --- 顶部 UI 区域：设置为 z-10，这样结算遮罩（z-20）会盖在它上面产生变暗效果 --- */}
            {(gameState === GameState.PLAYING || gameState === GameState.GAME_OVER) && (
                <div className="absolute inset-x-0 top-4 px-4 flex items-start pointer-events-none z-10">
                    <div className="flex flex-col font-mono font-bold text-sm text-gray-400 mt-2">
                        <div>$YGO PRICE: ${currentPrice.toFixed(4)}</div>
                        <div>SCORE: {score}</div>
                    </div>
                    <div className="absolute left-1/2 -translate-x-[50.5%] top-2">
                        <h1 className="text-4xl font-black text-[#E57D25] tracking-tight" style={{ fontFamily: 'monospace', textShadow: '4px 4px 0px #A34800' }}>$YGO PUMP</h1>
                    </div>
                    <div className="ml-auto mt-2"><img src="/logo.png" alt="Logo" className="h-6 w-auto object-contain opacity-70" /></div>
                </div>
            )}

            {/* 首页菜单 */}
            {gameState === GameState.MENU && (
                <div className="absolute inset-0 bg-black flex flex-col items-center text-white text-center p-6 z-25 overflow-hidden">
                    <div className="mt-8 flex justify-center"><img src="/logo.png" alt="Logo" className="h-8 w-auto object-contain" /></div>
                    <div className="mt-20"><h1 className="text-7xl font-black text-[#E57D25] tracking-tight" style={{ fontFamily: 'monospace', textShadow: '6px 6px 0px #A34800' }}>$YGO PUMP</h1></div>
                    <div className="mt-16 flex flex-col items-center">
                        <h2 className="text-3xl font-bold text-white mb-6 tracking-widest uppercase">MARKET CLIMBER</h2>
                        <div className="mb-10 text-base text-gray-300 font-mono tracking-widest leading-relaxed"><p>RISE WITH THE MARKET</p><p>AVOID DUMP BOTS & LASERS</p></div>
                        <button onClick={handleInitializePump} className="bg-[#E57D25] hover:bg-[#d46b1a] text-black font-bold py-5 px-16 rounded-none border-2 border-white transition-all text-2xl font-mono animate-pulse">INITIALIZE PUMP</button>
                        <div className="mt-14 flex flex-col items-center gap-4">
                            <div className="flex flex-col items-center gap-1.5"><div className="flex flex-col items-center gap-1.5"><KeyCap label="W" delay="0s" /><div className="flex gap-1.5"><KeyCap label="A" delay="0.4s" /><div className="w-10 h-10" /><KeyCap label="D" delay="0.8s" /></div></div></div>
                            <KeyCap label="SPACE" size="w-56" delay="1.2s" />
                        </div>
                    </div>
                    <div className="absolute bottom-8 left-0 right-0 opacity-40 text-[11px] font-mono tracking-widest uppercase">AI MEGA PLANT</div>
                </div>
            )}

            {isTransitioning && (
                <div className="absolute inset-0 bg-black z-50 flex items-center justify-center overflow-hidden">
                    <div className="pump-transition text-5xl font-black text-[#E57D25] font-mono tracking-tighter">PUMPING!!!</div>
                </div>
            )}

            {/* --- 结算界面：设置为 z-20，遮挡下方的 z-10 顶部 UI --- */}
            {gameState === GameState.GAME_OVER && (
                <div className="absolute inset-0 bg-black/85 flex flex-col justify-center items-center text-white z-20">
                    <h2 className="text-4xl font-bold mb-6 text-red-500 font-mono tracking-tighter uppercase">HOLD IT</h2>
                    <div className="flex flex-col items-center gap-4 mb-10">
                        <p className="text-3xl font-mono uppercase">Max Price: <span className="text-[#E57D25]">${currentPrice.toFixed(4)}</span></p>
                        <p className="text-3xl font-mono uppercase">Max Score: <span className="text-[#E57D25]">{score}</span></p>
                    </div>
                    <button onClick={resetGame} className="bg-white text-black hover:bg-gray-200 font-bold py-3 px-8 font-mono uppercase">REBOUND</button>
                </div>
            )}
        </div>
    );
